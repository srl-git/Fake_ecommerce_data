...
class Orders:
    ...
    def create(
        ...
        orders = self._generate_order_lines(
            items, all_user_ids, num_orders, max_num_items
        )
    ...
    def _get_active_products(self, products: Products) -> dict:
        try:
            ...
            raise ValueError(
                "ERROR in Orders.create(): Can't generate orders without any products in the database."
            )
    ...
    def _get_user_ids(
        self, users: Users, locales: list[str], num_orders: int
    ) -> list[int]:
    ...
    def _generate_order_lines(
        self, items: dict, user_ids: list[int], num_orders: int, max_num_items: int
    ) -> list[tuple]:
        item_skus = [item_sku for item_sku in items]
        item_popularities = [
            details["item_popularity"] for item_sku, details in items.items()
        ]
    ...
    def get_orders_by_date_range(
        ...
        with CloudSQLConnection() as db:
            db.cursor.execute(
                sql.order_statements.get_orders_by_date_range, (start_date, end_date)
            )
    ...
    def _save_to_cloud_storage(self, export_data: list[tuple], file_path: str):
        ...
        upload_to_bucket(
            f"order_reports/{file_path}", upload_data, os.getenv("STORAGE_BUCKET_NAME")
        )
    ...
    def _validate_create_args(
        self, users: Users, products: Products, num_orders: int, max_num_items: int
    ) -> None:
        ...
        if not (isinstance(num_orders, int) and num_orders > 0) or isinstance(
            num_orders, bool
        ):
            raise ValueError(
                "ERROR in Orders.create(). "
        ...
        if not (isinstance(max_num_items, int) and max_num_items > 0) or isinstance(
            max_num_items, bool
        ):
            raise ValueError(
                "ERROR in Orders.create(). "
