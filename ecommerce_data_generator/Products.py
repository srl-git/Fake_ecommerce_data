...
class Products:
    ...
    def update(
        ...
        if products_to_update:
            for product in products_to_update:
                ...
                price_updated = (
                    item_price[index] if item_price is not None else product[1]
                )
                date_updated = datetime.today().strftime("%Y-%m-%d")
                is_active_updated = (
                    is_active[index] if is_active is not None else product[5]
                )
                update_data.append(
                    (price_updated, date_updated, is_active_updated, sku_to_update)
                )

            with CloudSQLConnection() as db:
                db.cursor.executemany(
                    sql.product_statements.update_products, (update_data)
                )
    ...
    def get_products(
        self, item_sku: str | list[str] | tuple[str] | None = None
    ) -> list[tuple]:
        with CloudSQLConnection() as db:
            if item_sku:
                if not (
                    isinstance(item_sku, (str, list, tuple))
                    and all(isinstance(s, str) for s in item_sku)
                ):
                    raise ValueError(
                ...
                if isinstance(item_sku, str):
                    item_sku = (item_sku,)
                db.cursor.execute(
                    sql.product_statements.get_products_by_sku(item_sku), item_sku
                )
    ...
    def get_products_by_date_range(
        ...
        with CloudSQLConnection() as db:
            db.cursor.execute(
                sql.product_statements.get_products_by_date_range,
                (start_date, end_date),
            )
    ...
    def to_csv(
        ...
        if start_date or end_date:
            try:
                export_data = [
                    product[:-1]
                    for product in self.get_products_by_date_range(start_date, end_date)
                ]
    ...
    def _validate_update_args(
        ...
        is_active: bool | list[bool] | None,
    ) -> None:
        if not (
            item_sku
            and isinstance(item_sku, (list, str))
            and all(isinstance(s, str) for s in item_sku)
        ):
            raise ValueError(
        ...
        if isinstance(item_sku, list):
            invalid_skus = [
                sku
                for sku in item_sku
                if sku not in set(product[0] for product in self.get_products(item_sku))
            ]
        ...
        if isinstance(item_sku, str):
            invalid_skus = (
                item_sku
                if item_sku
                not in set(product[0] for product in self.get_products(item_sku))
                else None
            )
        ...
        if item_price is not None:
            if not isinstance(item_price, (list, int, float)) or isinstance(
                item_price, bool
            ):
                raise TypeError(
            ...
            if isinstance(item_price, list):
                ...
                if not all(
                    isinstance(element, (int, float)) and element >= 0
    ...
    def _get_sku_index(self, label_prefix: str) -> int:
        with CloudSQLConnection() as db:
            db.cursor.execute(
                sql.product_statements.get_sku_index, (f"{label_prefix}%",)
            )
    ...
    def _get_upper_limit(self) -> float:
        ...
        popularity_upper_limit = (
            max_popularity_score * 1.5 if max_popularity_score else 1
        )
    ...
    def _set_popularity_scores(self) -> None:
        ...
        new_popularity_scores = tuple(
            score * normalizer for score in old_popularity_scores
        )
        update_scores = zip(new_popularity_scores, old_popularity_scores)

        with CloudSQLConnection() as db:
            db.cursor.executemany(
                sql.product_statements.set_popularity_scores, (update_scores)
            )
